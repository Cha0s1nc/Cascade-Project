...
- **Lyrics translation** - auto-detects non-English tracks and shows a translate bar. One click translates to 11 languages. Detection and translation both run **entirely on your machine**: no lyric, and no record of what you are playing, ever leaves it. Language detection is trigram matching (`franc`); translation runs Helsinki-NLP's Marian models on ONNX Runtime in a background worker. The models ship with the app, so it works with no network at all. English is a direct translation; other targets pivot through English, which costs some fidelity. Korean is available as a source but not as a target, because no on-device model of this size covers English to Korean
